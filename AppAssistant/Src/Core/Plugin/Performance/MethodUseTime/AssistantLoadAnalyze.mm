...
@interface AssistantLoadAnalyze : NSObject
@end
@implementation AssistantLoadAnalyze
+(void)load{
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    BOOL needMethodUseTime = [defaults boolForKey:@"Assistant_method_use_time_key"];
    if (!needMethodUseTime) {
        return;
    }
    
    CFTimeInterval begin = CACurrentMediaTime();
    DLALog(@"DLA >>>> AssistantLoadAnalyze load start");
    if(!loadMS){
    ...
    DLALog(@"DLA >>>> AssistantLoadAnalyze load end , costs : %@",[NSString stringWithFormat:@"%@ - %@ms",NSStringFromClass([self class]), @(1000 * (end - begin))]);
}
@end
