...
extension Device {
    ...
    private static var deviceCode: String {
        ...
        guard let validating = NSString(bytes: &systemInfo.machine, length: Int(_SYS_NAMELEN), encoding: String.Encoding.ascii.rawValue)?.utf8String else {
            return ""
        }

        guard let versionCode = String(validatingUTF8: validating) else {
            return ""
        }
        return versionCode
    }
